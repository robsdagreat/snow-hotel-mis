...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate inputs
    if (empty($consumable_id) || empty($quantity) || empty($description) || empty($unit_price)) {
        $error_message = "All fields are required.";
    } else {
        // Get current stock to check if enough is available
        $current_stock = $stock->getStockByConsumableId($consumable_id);
        
        if (!$current_stock) {
            // If no stock record exists, create one with zero quantity
        ...
        if ($current_stock['quantity'] < $quantity) {
            $error_message = "Not enough stock available. Current quantity: " . $current_stock['quantity'];
        } else {
            // Record consumption
            $result = $stock->updateStock(
                $consumable_id,
            ...
            if ($result) {
                ...
                $error_message = "Failed to record consumption.";
            }
        }
    }
}
                    ...
                    <div class="form-group">
                        ...
                        <select name="consumable_id" id="consumable_id" class="form-control" required>
                            ...
                            <?php foreach ($all_consumables as $item): ?>
                                <option value="<?php echo $item['id']; ?>"><?php echo htmlspecialchars($item['item']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function fetchCurrentStock(consumableId) {
                ...
                try {
                    ...
                    if (data.success) {
                        stockQuantity.textContent = data.quantity;
                        currentStockInfo.classList.add('visible');
                    } else {
